  ...
  return (
    <div className="font-sans text-gray-800 w-full h-full flex flex-row">
      ...
      <div className="w-[32%] bg-slate-800 text-white p-6 sidebar-break">
        ...
        <h2 className="text-[13px] font-medium text-emerald-400 mb-8">{experience[0]?.position || 'Professional'}</h2>
            ...
            <div className="space-y-4">
              {education.map(edu => (
                <div key={edu.id}>
                  ...
                  <div className="text-[11px] text-emerald-400 mb-1">{edu.field}</div>
      ...
      <div className="w-[68%] p-6 bg-slate-50">
        {summary && (
          <div className="mb-8 section-break">
            <h3 className="text-[15px] font-bold text-slate-800 mb-3 flex items-center gap-2">
               <span className="w-6 h-0.5 bg-emerald-500 inline-block"></span> Profile
            </h3>
        ...
        {experience.length > 0 && (
          <div className="mb-8 section-break">
            <h3 className="text-[15px] font-bold text-slate-800 mb-5 flex items-center gap-2">
               <span className="w-6 h-0.5 bg-emerald-500 inline-block"></span> Experience
            </h3>
            <div className="space-y-6">
              {experience.map(exp => (
                <div key={exp.id} className="relative pl-4 border-l-2 border-slate-200 item-break">
                  <div className="absolute w-2.5 h-2.5 bg-emerald-500 rounded-full -left-[6px] top-1.5 border-2 border-slate-50"></div>
                  ...
                  <div className="text-[13px] text-emerald-600 font-medium mb-2">{exp.company}</div>
                  ...
                  <div className="text-[13px] text-slate-600 space-y-1">
                    {exp.highlights.map((item, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <span className="text-emerald-400 mt-[0.2em] leading-none text-[1em]">▹</span>
        ...
        {projects.length > 0 && (
          <div className="section-break">
            <h3 className="text-[15px] font-bold text-slate-800 mb-5 flex items-center gap-2">
               <span className="w-6 h-0.5 bg-emerald-500 inline-block"></span> Projects
            </h3>
            <div className="grid grid-cols-1 gap-4">
              {projects.map(proj => (
                <div key={proj.id} className="bg-white p-4 rounded-lg shadow-sm border border-slate-100 item-break">
                  ...
                  <div className="text-[11px] text-emerald-500 font-medium mb-2">{(proj.technologies || []).join(' • ')}</div>
                  <div className="text-[13px] text-slate-600 space-y-1">
        ...
        {sections.length > 0 && sections.map(section => (
          <div key={section.id} className="mb-6 item-break">
            <h2 className="text-sm font-bold text-slate-800 uppercase tracking-widest border-b-2 border-emerald-500 pb-1 mb-4">
              {section.title}
            </h2>
            <div className="text-[13px] text-slate-600 space-y-1">
